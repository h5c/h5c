...
uint256 hashGenesisBlock("0x000008c017614d87f1becd90d182ee403d82e3e91a0306c22a83812e8b0b0fe2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HighFiveCoin: starting difficulty is 1 / 2^12
static CBigNum bnInitialDifficulty(~uint256(0) >> 31);
...
const int BLOCK_BEFORE_START_DGW2 = 8000;
const int BLOCK_BEFORE_START_DGW2_FTESTNET = 5;
const int BLOCK_BEFORE_START_DIGISHIELD = 40000;
const int BLOCK_BEFORE_START_DIGISHIELD_FTESTNET = 10;

/** Fees smaller than this (in satoshi) are considered zero fee (for transaction creation) */
int64 CTransaction::nMinTxFee = 0.0002 * COIN;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 0.0002 * COIN;
...
const string strMessageMagic = "HighFiveCoin Signed Message:\n";
...
{
    // HighFiveCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    int weeksElapsed = nHeight / 13440;
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    while (nTime > 0 && bnResult < bnProofOfWorkLimit)
    {
        if(nBestHeight+1<BLOCK_BEFORE_START_DIGISHIELD){
            // Maximum 400% adjustment...
            bnResult *= 4;
            // ... in best-case exactly 4-times-normal target time
            nTime -= nTargetTimespan*4;
...
unsigned int static GetNextWorkRequiredOld(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    static const int64	BlocksTargetSpacing			= 1.5 * 60;
    unsigned int		TimeDaySeconds				= 60 * 60 * 24;
    int64				PastSecondsMin				= TimeDaySeconds * 1.5 * 0.1;
    int64				PastSecondsMax				= TimeDaySeconds * 1.5 * 2.8;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{	
    if ((pindexLast->nHeight > BLOCK_BEFORE_START_DIGISHIELD) || fTestNet)
	return Digishield(pindexLast, pblock);
    if ((pindexLast->nHeight > BLOCK_BEFORE_START_DGW2) || fTestNet)
	return DarkGravityWave2(pindexLast, pblock);
    return GetNextWorkRequiredOld(pindexLast, pblock);
}
...
{
    // HighFiveCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000d7fdd270be3cdbb9a6c17dd89aa0148db7a06eaae6fe8c0eaa669ce4e6c");
    }
...
bool InitBlockIndex() {
	...
	const char* pszTimestamp = "Gimme a HighFive now! H5C";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 400 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184610fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime = 1396606028;
        block.nBits = 0x1e0ffff0;
        block.nNonce = 1205025;

        if (fTestNet)
        {
            block.nTime    = 1396606027;
            block.nNonce   = 1371755;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x9224cbd500333213a0203b0ceca80a0ba8e4d4c74a41fedaea0449dc9e040188"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc0, 0xb8, 0xde }; // HighFiveCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HighFiveCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HighFiveCoinMiner : ProcessBlock, block not accepted");
    }
